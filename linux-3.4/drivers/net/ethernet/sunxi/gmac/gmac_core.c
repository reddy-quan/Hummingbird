...
{
	//hanbiao add for gmac
       script_item_u gmac_pio; 
	 int ret = -1;
	...
	if (!gmac_used.val) {
	 ...
	 //hanbiao add for gmac
    if (SCIRPT_ITEM_VALUE_TYPE_PIO !=script_get_item("3g_para", "gmac_pwr", &gmac_pio))
    {
		printk("ERR:gmac_pwr failed\n");
    }


    //hanbiao add for gmac
    ret = gpio_request(gmac_pio.gpio.gpio, "gmac_pwr");
     if(ret != 0){
            printk("gpio_request gmac_pwr failed\n");
        }else{
            /* set config, ouput */
             sw_gpio_setcfg(gmac_pio.gpio.gpio, GPIO_CFG_OUTPUT);
	      __gpio_set_value(gmac_pio.gpio.gpio, 1);
        }
